...
def exp_type_to_hyperparameters(args):
    ...
    if args.exp_type == 'bmi':
        ...
        p = {'Ntraining': [1,2,3,4,5,9,13,18], 
             'rotation': range(20),
             #'lrate': [0.001, 0.0001, 0.00001, 0.000001, 0.0000001]
             }
    else: 
...
def execute_exp(args=None):
   ...
   # Generate log data
    ...
    data = pd.DataFrame({
    "Timestamp": time_testing.flatten(),  # Flatten in case the timestamps array is 2D
    "True Acceleration": outs_testing.flatten(),  # Flatten in case it's 2D, adjust if necessary based on your data structure
    "Predicted Acceleration": results['predict_testing'].flatten()  # Flatten in case it's 2D, adjust if necessary
    })

    # Log the DataFrame as a table in WandB
    wandb.log({"Test Predictions": wandb.Table(dataframe=data)})

    wandb.log({"True vs Predicted Acceleration": wandb.plot.line(data, "Timestamp", 
                                                             ["True Acceleration", "Predicted Acceleration"],
                                                             title="True vs Predicted Acceleration",
                                                             xname="Timestamp")})
